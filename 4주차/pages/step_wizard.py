...
if st.session_state.step == 1:
    ...
    # 기본값을 session_state에서 가져와서 유지되도록 설정
    name = st.text_input('이름', value=st.session_state.get('saved_name', ''))
    age = st.number_input('나이', min_value=1, max_value=100, value=st.session_state.get('saved_age', 20))
    
    if st.button('다음 →'):
        if name: # 이름 입력 확인
            st.session_state.saved_name = name # 데이터 안전하게 저장
...
# ---- Step 2: 관심 분야 ----
    ...
    with col2:
        if st.button('다음 →'):
            st.session_state.saved_interests = interests # 관심사 저장
